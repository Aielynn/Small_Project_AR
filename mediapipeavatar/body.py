...
class BodyThread(threading.Thread):
    ...
    def run(self):
        ...
        with mp_pose.Pose(min_detection_confidence=0.80, min_tracking_confidence=0.5, model_complexity = global_vars.MODEL_COMPLEXITY,static_image_mode = False,enable_segmentation = True) as pose: 
            ...
            while not global_vars.KILL_THREADS and capture.cap.isOpened():
                ...
                #while(self.frame_captured == False):
                #    try:
                #        self.receive_data()
                #    except Exception as ex:  
                #        print("Failed to receive frame data, is the Unity project open?")
                ...
                ret = capture.ret
                image = capture.frame
                #image = cv2.imdecode(self.input_frame, -1)
    ...
    def receive_data(self):
        if not global_vars.USE_LEGACY_PIPES:
            self.input_frame = self.client.receiveMessage()

            if self.input_frame != None:
            ...
            if self.pipe != None:
                try:
                    self.input_frame = self.pipe.recv(1024)

                    if self.input_frame !=None:
                        self.frame_captured= True
                except Exception as ex:  
                    print("Failed to read from pipe. Is the unity project open?")
                    self.pipe= None
        pass   
