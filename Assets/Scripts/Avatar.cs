...
public class Avatar : MonoBehaviour
{
    ...
    private Vector3 initialLFootPos;
    private Vector3 initialRFootPos;
    private Vector3 currAvgFootPos;
    ...
    public delegate void PrepareCalibration();
    public static event PrepareCalibration onPrepareCalibration;

    private void Start()
    {
        UnityEngine.Debug.Log("Avatar Start Function");
        //PhaseManager.onPrepareCalibration += StartAvatar;
        initialRotation = transform.rotation;
        initialPosition = transform.position;
        parentCalibrationData = new Dictionary<HumanBodyBones, CalibrationData>();
        
        Calibrated = false;

        if (calibrationData && useCalibrationData)
        {
            UnityEngine.Debug.Log("From persistent");
        ...
        UnityEngine.Debug.Log("Start Avatar Function");
        UnityEngine.Debug.Log(server);
        UnityEngine.Debug.Log(animator);
        UnityEngine.Debug.Log(spineUpDown);

        onPrepareCalibration.Invoke();
    }
    ...
    public void CalibrateFromPersistent()
    {
        ...
        Calibrated = true;
        initialLFootPos = animator.GetBoneTransform(HumanBodyBones.LeftFoot).position;
        initialRFootPos = animator.GetBoneTransform(HumanBodyBones.RightFoot).position;
    ...
    public void Calibrate()
    {
        ...
        UnityEngine.Debug.Log(animator);
        // UnityEngine.Debug.Log(HumanBodyBones);
        UnityEngine.Debug.Log(server);
    ...
    private float calcVerticalDisplacement()
    {
        float displacement = 0;
        Vector3 lFootPos = animator.GetBoneTransform(HumanBodyBones.LeftFoot).position;
        Vector3 rFootPos = animator.GetBoneTransform(HumanBodyBones.RightFoot).position;
        float diffL = lFootPos.y; // - initialLFootPos.y;
        float diffR = rFootPos.y; // - initialRFootPos.y;

        //get minimum distance
        float minDiff = Mathf.Min(diffL, diffR);
    ...
    private bool withinRange(Vector3 refPos)
    {
    ...
    private void Update()
    {
        ...
        if(parentCalibrationData.Count > 0)
        {
            ...
            //transform.position = Vector3.Lerp(transform.position, initialPosition + Vector3.down * displacement + Vector3.up * footGroundOffset,
            //    Time.deltaTime*5f);

            float vDisplacement = calcVerticalDisplacement();
            //Vector3 hDisplacement = calcHorizontalDisplacement();

            transform.position = Vector3.Lerp(transform.position, initialPosition + Vector3.down * vDisplacement,
            Time.deltaTime*20f);   
        }
